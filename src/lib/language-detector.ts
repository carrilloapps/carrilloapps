...
export function detectLanguage(code: string): string {
  ...
  if (
    (/:\s*(string|number|boolean|any|void|never|unknown|object|Promise)\b/.test(code) &&
  ...
  if (
    (/func\s+\w+\s*\([^)]*\)/.test(code) &&
      (code.includes("->") || /var\s+\w+:\s*\w+/.test(code))) ||
  ...
  if (
    ...
    /^\$\s+/.test(firstLine) ||
    /^(npm|yarn|pnpm|git|docker|kubectl|cargo|go|node|python|pip|brew|apt|yum|dnf|pacman|systemctl|service|curl|wget|ssh|scp|rsync|tar|unzip|zip|chmod|chown|mkdir|cd|ls|pwd|rm|mv|cp|cat|grep|sed|awk|find|ps|kill|sudo|su)\s+/.test(
      firstLine,
    ) ||
    lines.filter((line) =>
      /^(npm|yarn|pnpm|git|docker|kubectl|cargo|go|node|python|pip|brew|apt|yum|dnf|pacman|systemctl|service|curl|wget|ssh|scp|rsync|tar|unzip|zip|chmod|chown|mkdir|cd|ls|pwd|rm|mv|cp|cat|grep|sed|awk|find|ps|kill|sudo|su)\s+/.test(
  ...
  if (
    ...
    (/<\/\w+>/.test(code) &&
      ...
      !code.includes("()") &&
      !code.includes("func ")) ||
    (/<(div|span|p|a|img|header|footer|section|nav|main|article|aside)[^>]*>/.test(code) &&
      /<\//.test(code))
  ) {
    return "markup"
  }
  ...
  if (
    ...
    return code.includes("@mixin") || code.includes("@include") || code.includes("@extend")
      ? "scss"
      : "css"
  }
