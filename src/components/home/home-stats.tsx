...
export function HomeStats({
  ...
  return (
    <section
      className="relative border-y border-zinc-800/50 bg-zinc-950/40 py-12 backdrop-blur-sm md:py-16"
      role="region"
      aria-label={ariaLabel}
    >
      <div className="relative z-10 container mx-auto px-4">
        <div className="grid grid-cols-2 gap-x-6 gap-y-10 md:grid-cols-4 md:gap-x-12">
          {metrics.map((stat) => (
            <div key={stat.label} className="space-y-2">
              <div className="text-5xl font-extrabold tracking-tighter text-white tabular-nums md:text-6xl lg:text-7xl">
                {stat.value}
              </div>
              <div className="max-w-[18ch] text-sm leading-snug text-zinc-400 md:text-base">
                {stat.label}
              </div>
            </div>
          ))}
