...
export function MediaCard({ image, alt, title, children, sizes, className }: MediaCardProps) {
  return (
    <SurfaceCard
      as="article"
      className={cn(
        "group flex h-full flex-col overflow-hidden transition-transform duration-300 ease-out hover:-translate-y-1",
        className,
      )}
    >
      <div className="relative aspect-[16/10] overflow-hidden">
        ...
        <div className="absolute inset-x-5 bottom-4 flex items-center gap-3">
          <span
            ...
            aria-hidden="true"
          />
          <h3 className="text-xl leading-tight font-bold tracking-tight text-white drop-shadow-md md:text-2xl">
            {title}
          </h3>
        </div>
      </div>
