...
const SIZES = {
  sm: {
    tile: "px-2.5 py-4 min-h-[96px]",
    value: "text-xl md:text-2xl",
    label: "text-[10px] md:text-[11px]",
  },
  md: {
    tile: "px-3 py-5 md:px-4 min-h-[116px]",
...
export function StatTiles({
  ...
  return (
    <ul
      className={cn(
        "m-0 grid list-none gap-2 p-0 md:gap-3",
        COLUMNS[columns],
    ...
    >
      {metrics.slice(0, columns).map((metric) => (
        ...
        >
          ...
          <span
            className={cn(
              "leading-tight text-zinc-300",
              s.label,
              plain && "tracking-wide uppercase",
            )}
          >
            {metric.label}
          </span>
