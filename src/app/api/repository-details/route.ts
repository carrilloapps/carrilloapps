...
const getCachedGitLabRepoDetails = unstable_cache(
  async (projectId: string) => {
    ...
    try {
      const commitsResponse = await fetch(
        `https://gitlab.com/api/v4/projects/${encodeURIComponent(projectId)}/repository/commits?per_page=1`,
        {
          headers,
        },
      )
    ...
    try {
      const tagsResponse = await fetch(
        `https://gitlab.com/api/v4/projects/${encodeURIComponent(projectId)}/repository/tags?per_page=1`,
        {
          headers,
        },
      )
